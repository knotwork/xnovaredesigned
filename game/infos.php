...
$xnova_root_path = './';
include_once($xnova_root_path . 'extension.inc');
include_once($xnova_root_path . 'common.' . $phpEx);
...
function ShowBuildingInfoPage ($CurrentUser, $CurrentPlanet, $BuildID) {
	...
	if       ($BuildID >=   1 && $BuildID <=   3) {
		...
		// Laboratoire
		$PageTPL              = gettemplate('info_buildings_general');
	} elseif ($BuildID >= 202 && $BuildID <= 218) {
		// Flotte
	...
	} elseif ($BuildID >= 301 && $BuildID <= 399) {
		// Defenses
		$PageTPL              = gettemplate('info_buildings_caserne');
		$parse['element_typ'] = $lang['tech'][300];
...
//
	...
	$page = ShowBuildingInfoPage ($user, $planetrow, $gid);
        ...
        if($_GET['axah']){
                ...
                displaypage($page, $lang['nfo_page_title']);
        }
