...
function ProductionRates ($CurrentUser,$CurrentPlanet,$basic=false) {
	...
	*/
	...
	if($basic){
		...
		//2 Check storage
		$Caps['metal_max']	 = BASE_STORAGE_SIZE + ((BASE_STORAGE_SIZE / 2) * floor(pow(2,$CurrentPlanet[$resource[22]])));
		$Caps['crystal_max']   = BASE_STORAGE_SIZE + ((BASE_STORAGE_SIZE / 2) * floor(pow(2,$CurrentPlanet[$resource[23]])));
		$Caps['deuterium_max'] = BASE_STORAGE_SIZE + ((BASE_STORAGE_SIZE / 2) * floor(pow(2,$CurrentPlanet[$resource[24]])));
